...
public class PacketMMI
{
    ...
    private MMIVariables mmivariable;
    
    public PacketMMI() {
        this.mmivariable = new MMIVariables();
    }
    
    public int MMI_START_ATP(final byte[] bd) {
        return this.mmivariable.MMI_M_START_REQ(bd[3]);
    }
    
    public Vector MMI_DYMANIC(final byte[] bd) {
    ...
    public Vector MMI_STATUS(final byte[] bd) {
        final Vector tmp = new Vector();
    ...
    public Vector MMI_CURRENT_DRIVER_DATA(final byte[] bd) {
        final Vector rtn = new Vector();
        final byte[] driverid = { bd[3], bd[4], bd[5], bd[6], bd[7], bd[8], bd[9], bd[10] };
        rtn.add(this.mmivariable.MMI_NID_DRIVER(driverid));
        final byte[] oper = { bd[11], bd[12], bd[13], bd[14] };
        rtn.add(this.mmivariable.MMI_NID_OPERATION(oper));
        final byte[] ws = { bd[15], bd[16], bd[17], bd[18], bd[19], bd[20], bd[21], bd[22] };
        rtn.add(this.mmivariable.MMI_NID_WORKSHIFT(ws));
        return rtn;
    }
    
    public String MMI_FORCED_DRIVER_REQUEST(final byte[] bd) {
        return this.mmivariable.MMI_M_FORCED_REQUEST(bd[3], bd[4]);
    }
    
    public Vector MMI_CURRENT_TRAIN_DATA(final byte[] bd) {
        final Vector trainData = new Vector();
        ...
        int pos = 13;
        int len = bd[pos];
        final byte[] traction = new byte[len * 2];
        ++pos;
        ...
        pos += len * 2;
        len = bd[pos];
        final byte[] dsguarant = new byte[len * 4];
        ++pos;
        ...
        pos += len * 4;
        len = bd[pos];
        final byte[] deguarant = new byte[len * 4];
        ++pos;
        for (int k = 0; k < len; ++k) {
        ...
        trainData.add(new Integer(this.mmivariable.MMI_T_CUTOFFTRACTION(bd[++pos], bd[++pos])));
        ...
        trainData.add(new String(this.mmivariable.MMI_NID_ENGINE(bd[++pos], bd[++pos], bd[++pos])));
    ...
    public Vector MMI_ECHOED_TRAIN_DATA(final byte[] bd) {
        final Vector rtn = new Vector();
        rtn.add(new Integer(this.mmivariable.MMI_V_MAXTRAIN_x((byte)~bd[3], (byte)~bd[4])));
        rtn.add(new Integer(this.mmivariable.MMI_V_MAXTRAIN_x((byte)~bd[5], (byte)~bd[6])));
        rtn.add(new Integer(this.mmivariable.MMI_V_MAXTRAIN_x((byte)~bd[7], (byte)~bd[8])));
        rtn.add(new Integer(this.mmivariable.MMI_V_MAXTRAIN_x((byte)~bd[9], (byte)~bd[10])));
        rtn.add(new Integer(this.mmivariable.MMI_M_TRAIN_TYPE((byte)~bd[11], (byte)~bd[12])));
        rtn.add(this.mmivariable.MMI_NID_ENGINE((byte)~bd[13], (byte)~bd[14], (byte)~bd[15]));
        rtn.add(new Integer(this.mmivariable.MMI_A_MAX_DEC((byte)~bd[16], (byte)~bd[17])));
        rtn.add(new Integer(this.mmivariable.MMI_A_MAX_ACC((byte)~bd[18], (byte)~bd[19])));
        rtn.add(new Integer(this.mmivariable.MMI_T_CUTOFFTRACTION((byte)~bd[20], (byte)~bd[21])));
        rtn.add(new Integer(this.mmivariable.MMI_T_BRAKE_EB((byte)~bd[22], (byte)~bd[23])));
        rtn.add(new Integer(this.mmivariable.MMI_T_BRAKE_SB((byte)~bd[24], (byte)~bd[25])));
        int pos = 26;
        int len = ~bd[pos];
        final byte[] degu = new byte[len * 4];
        ++pos;
        for (int i = 0; i <= len - 1; ++i) {
            degu[i * 4 + 0] = (byte)~bd[pos + i * 4 + 0];
            degu[i * 4 + 1] = (byte)~bd[pos + i * 4 + 1];
            degu[i * 4 + 2] = (byte)~bd[pos + i * 4 + 2];
            degu[i * 4 + 3] = (byte)~bd[pos + i * 4 + 3];
        }
        this.mmivariable.MMI_N_DEGUARANT(degu);
        pos += len * 4;
        len = ~bd[pos];
        final byte[] dsgu = new byte[len * 4];
        ++pos;
        for (int j = 0; j <= len - 1; ++j) {
            dsgu[j * 4 + 0] = (byte)~bd[pos + j * 4 + 0];
            dsgu[j * 4 + 1] = (byte)~bd[pos + j * 4 + 1];
            dsgu[j * 4 + 2] = (byte)~bd[pos + j * 4 + 2];
            dsgu[j * 4 + 3] = (byte)~bd[pos + j * 4 + 3];
        }
        this.mmivariable.MMI_N_DSGUARANT(dsgu);
        pos += len * 4;
        len = ~bd[pos];
        final byte[] trac = new byte[len * 2];
        ++pos;
        for (int k = 0; k <= len - 1; ++k) {
            trac[k * 2 + 0] = (byte)~bd[pos + k * 2 + 0];
            trac[k * 2 + 1] = (byte)~bd[pos + k * 2 + 1];
        }
        this.mmivariable.MMI_N_TRACTION(trac);
        pos += len * 2;
        rtn.add(new Double(this.mmivariable.MMI_M_AXLELOAD((byte)~bd[pos])));
        ++pos;
        this.mmivariable.MMI_M_LOADINGGAUGE();
        rtn.add(new Integer(this.mmivariable.MMI_V_MAXTRAIN((byte)~bd[++pos], (byte)~bd[++pos])));
        rtn.add(new Integer(this.mmivariable.MMI_L_TRAIN((byte)~bd[++pos], (byte)~bd[++pos])));
        rtn.add(this.mmivariable.MMI_NC_TRAIN((byte)~bd[++pos], (byte)~bd[++pos]));
        rtn.add(this.mmivariable.MMI_M_DATA_EDIT_ENABLE((byte)~bd[++pos], (byte)~bd[++pos]));
        return rtn;
    }
    
    public void MMI_TRACK_DESCRIPTION(final byte[] bd) {
    }
    
    public Vector MMI_DRIVER_MESSAGE(final byte[] bd) {
        final Vector rtn = new Vector();
    ...
    public Vector MMI_FAILURE_REPORT_ATP(final byte[] bd) {
        final Vector rtn = new Vector();
    ...
    public int MMI_TEST_REQUEST(final byte[] bd) {
        return this.mmivariable.MMI_M_TEST_(bd[3], bd[4]);
    }
    
    public long MMI_SET_TIME_ATP(final byte[] bd) {
        return this.mmivariable.MMI_T_UTC(bd[3], bd[4], bd[5], bd[6]);
    }
    
    public Vector MMI_CURRENT_SR_RULES(final byte[] bd) {
        final Vector SR = new Vector(2);
        SR.add(new Integer(this.mmivariable.MMI_V_STFF(bd[3], bd[4])));
        SR.add(new Long(this.mmivariable.MMI_L_STFF(bd[5], bd[6], bd[7], bd[8])));
        return SR;
    }
    
    public Vector MMI_ECHOED_SR_RULES(final byte[] bd) {
        final Vector rule = new Vector();
        rule.add(new Long(this.mmivariable.MMI_L_STFF((byte)~bd[3], (byte)~bd[4], (byte)~bd[5], (byte)~bd[6])));
        rule.add(new Integer(this.mmivariable.MMI_V_STFF((byte)~bd[7], (byte)~bd[8])));
        return rule;
    }
    
    public Vector MMI_GEO_POSITION(final byte[] bd) {
        final Vector position = new Vector(2);
    ...
    public void MMI_SELECT_STM_REQUEST(final byte[] bd) {
    }
    
    public void MMI_STM_DATA_TO_CAB_1(final byte[] bd) {
    }
    
    public void MMI_STM_DATA_TO_CAB_2(final byte[] bd) {
    }
    
    public void MMI_RU_DATA(final byte[] bd) {
    }
    
    public Vector MMI_START_MMI(final byte[] bd) {
        final Vector info = new Vector(5);
    ...
    public int MMI_STATUS_REPORT(final byte[] bd) {
        return this.mmivariable.MMI_M_MMI_STATUS(bd[3]);
    }
    
    public int MMI_DRIVER_REQUEST(final byte[] bd) {
        return this.mmivariable.MMI_M_REQUEST(bd[3]);
    }
    
    public Vector MMI_NEW_DRIVER_DATA(final byte[] bd) {
        return this.MMI_CURRENT_DRIVER_DATA(bd);
    }
    
    public Vector MMI_NEW_TRAIN_DATA(final byte[] bd) {
        return this.MMI_CURRENT_TRAIN_DATA(bd);
    }
    
    public Vector MMI_CONFIRMED_TRAIN_DATA(final byte[] bd) {
        return this.MMI_ECHOED_TRAIN_DATA(bd);
    }
    
    public int MMI_FAILURE_REPORT_MMI(final byte[] bd) {
        return this.mmivariable.MMI_I_FAILURE_NUMBER(bd[3], bd[4]);
    }
    
    public Vector MMI_DRIVER_MESSAGE_ACK(final byte[] bd) {
        final Vector tmp = new Vector();
    ...
    public int MMI_FAILURE_REPORT_ACK(final byte[] bd) {
        return this.mmivariable.MMI_I_FAILURE_NUMBER(bd[3], bd[4]);
    }
    
    public long MMI_SET_TIME_MMI(final byte[] bd) {
        return this.mmivariable.MMI_T_UTC(bd[3], bd[4], bd[5], bd[6]);
    }
    
    public int MMI_TEST_RESULT(final byte[] bd) {
        return this.mmivariable.MMI_M_TEST_(bd[3], bd[4]);
    }
    
    public Vector MMI_NEW_SR_RULES(final byte[] bd) {
        return this.MMI_CURRENT_SR_RULES(bd);
    }
    
    public Vector MMI_CONFIRMED_SR_RULES(final byte[] bd) {
        return this.MMI_ECHOED_SR_RULES(bd);
    }
    
    public Vector MMI_NEW_RBC_DATA(final byte[] bd) {
        final Vector tmp = new Vector();
        tmp.add(new Integer(this.mmivariable.MMI_NID_RBC(bd[3], bd[4], bd[5])));
        final byte[] radio = new byte[7];
        for (int i = 0; i <= 7; ++i) {
            radio[i] = bd[7 + i];
        }
    ...
    public void MMI_SELECT_STM(final byte[] db) {
        this.mmivariable.MMI_NID_STM(db[0], db[1]);
    }
    
    public void MMI_STM_DATA_FROM_CAB_1(final byte[] db) {
    }
    
    public void MMI_STM_DATA_FROM_CAB_2(final byte[] db) {
    }
    
    public void MMI_RU_DATA_FROM_CAB_1(final byte[] db) {
    }
    
    public void MMI_RU_DATA_FROM_CAB_2(final byte[] db) {
    }
}
