...
public class GradientProfile extends DefaultTelegram {
  ...
  public GradientProfile(byte[] data) {
    super(data);
    go();
  }
  ...
  private void go() {
    try {
      ...
      tmpdata = HexCode.getBit(getCode(), 49, 54);
      this.N_ITER = Byte2Number.getUnsigned(tmpdata[0]);
      this.iter = new int[get_N_ITER() + 1][3];
      this.iter[0] = getNext(0);
      for (int x = 1; x <= get_N_ITER(); x++)
        this.iter[x] = getNext(x); 
    } catch (Exception ex) {
      ex.printStackTrace();
    } 
  }
